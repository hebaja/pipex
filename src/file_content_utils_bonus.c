...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   file_content_utils_bonus.c                         :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: hebatist <hebatist@student.42.rio>         +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/01/23 18:57:44 by hebatist          #+#    #+#             */
/*   Updated: 2025/02/01 13:20:46 by hebatist         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../include/pipex_bonus.h"

int	fill_file_content(const char *in_filename, char *buffer, char *file_content)
{
	int			fd;
	int			i;
	int			bytes;
...
char	*get_file_content(const char *in_filename)
{
	...
	file_content = (char *)malloc(sizeof(char)
			* (get_file_len(in_filename) + 1));
	buffer = (char *)malloc(sizeof(char) * 10);
	if (file_content && buffer)
	{
		if (fill_file_content(in_filename, buffer, file_content))
			return (file_content);
		else
			return (NULL);
	}
